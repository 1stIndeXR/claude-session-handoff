...
replay any "Verbatim-Kept Threads" unchanged, confirm the restore to the user
in one line, and immediately resume the "Immediate next action" from "Refresh
Continuation" (or section 7) without waiting for confirmation. If the marker
is stale or absent, do nothing.
```
