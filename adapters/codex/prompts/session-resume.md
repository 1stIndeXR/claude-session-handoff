...
3. Read `latest.md` in full, plus every file listed in section 2 ("Key Files & Documentation"). Replay any "Verbatim-Kept Threads" unchanged, preserving exact text, order, and speaker labels.
4. **Hot:** delete the marker, confirm in one line (`Refreshed — resuming: <immediate next action from Refresh Continuation>`), then resume that action immediately without waiting.
