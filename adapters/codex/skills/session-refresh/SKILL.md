...
Modes are independent: `aggressive` controls memory writes, while `selective` classifies conversation threads into verbatim-kept, summarized, and omitted buckets. Both arguments may be supplied. On restore, replay verbatim-kept threads unchanged before resuming the immediate next action.
