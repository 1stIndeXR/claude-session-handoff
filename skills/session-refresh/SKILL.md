...
description: Refresh the current session in place — write a comprehensive handoff, have the user compact, then auto-restore state so work continues in the same session with a small context. Trigger when the user invokes /session-refresh, says "refresh the session", "renew the session", or wants to keep working but context is getting heavy (30%+ used). Distinct from session-handoff, which ends a session.
---

# Session Refresh (Claude Code)

Follow the core procedure in `core/session-refresh.md` (sibling of `skills/` at the plugin root) exactly. Claude Code specifics:

- **Mode:** conservative by default; `aggressive` argument passes through to the handoff step.
- **Closing line (step 4):** this plugin ships a `SessionStart(compact)` hook that auto-injects `handoffs/latest.md`, so use the auto-restore variant: *"Now run `/compact` — state will be restored automatically after compaction."*
